...
#define HW_BRINGUP_TEST_SWITCHES                                               \
  0 /* 1 = run switch test, bypass RTOS. Set to 0 to resume normal startup. */
#define HW_BRINGUP_TEST_I2C_SCAN                                               \
  0 /* 1 = run I2C scan test, bypass RTOS. Set to 0 to resume normal startup.  \
     */
#define HW_BRINGUP_TEST_LCD_MESSAGE                                            \
  0 /* 1 = run LCD message test, bypass RTOS. Set to 0 to resume normal        \
       startup. */
#define HW_BRINGUP_TEST_RELAYS                                                 \
  0 /* 1 = run relay test, bypass RTOS. Set to 0 to resume normal startup. */
#define HW_BRINGUP_TEST_NTC                                                    \
  0 /* 1 = run NTC sensor test, bypass RTOS. Set to 0 to resume normal         \
       startup. */
...
int main(void) {
...
#if HW_BRINGUP_TEST_SWITCHES
  while (1) {
    g_upPressed = (HAL_GPIO_ReadPin(UpArrowSwitch_GPIO_Port,
                                    UpArrowSwitch_Pin) == GPIO_PIN_RESET);
    g_downPressed = (HAL_GPIO_ReadPin(DownArrowSwitch_GPIO_Port,
                                      DownArrowSwitch_Pin) == GPIO_PIN_RESET);
    g_modePressed = (HAL_GPIO_ReadPin(ModeSwitch_GPIO_Port, ModeSwitch_Pin) ==
                     GPIO_PIN_RESET);
    g_enterPressed = (HAL_GPIO_ReadPin(EnterSwitch_GPIO_Port,
                                       EnterSwitch_Pin) == GPIO_PIN_RESET);

    if (g_upPressed || g_downPressed || g_modePressed || g_enterPressed) {
      HAL_GPIO_WritePin(OnOffLed_GPIO_Port, OnOffLed_Pin,
                        GPIO_PIN_SET); /* any press -> LED on */
    } else {
      HAL_GPIO_WritePin(OnOffLed_GPIO_Port, OnOffLed_Pin,
                        GPIO_PIN_RESET); /* nothing pressed -> LED off */
    }
  }
#endif
...
#if HW_BRINGUP_TEST_RELAYS
  while (1) {
    uint8_t upPressed = (HAL_GPIO_ReadPin(UpArrowSwitch_GPIO_Port,
                                          UpArrowSwitch_Pin) == GPIO_PIN_RESET);
    uint8_t downPressed =
        (HAL_GPIO_ReadPin(DownArrowSwitch_GPIO_Port, DownArrowSwitch_Pin) ==
         GPIO_PIN_RESET);
    uint8_t modePressed = (HAL_GPIO_ReadPin(ModeSwitch_GPIO_Port,
                                            ModeSwitch_Pin) == GPIO_PIN_RESET);
    uint8_t enterPressed =
        (HAL_GPIO_ReadPin(EnterSwitch_GPIO_Port, EnterSwitch_Pin) ==
         GPIO_PIN_RESET);
    ...
    HAL_GPIO_WritePin(HeatRelay_GPIO_Port, HeatRelay_Pin,
                      g_heatRelayOn ? GPIO_PIN_RESET : GPIO_PIN_SET);
    HAL_GPIO_WritePin(FanRelay_GPIO_Port, FanRelay_Pin,
                      g_fanRelayOn ? GPIO_PIN_RESET : GPIO_PIN_SET);
    HAL_GPIO_WritePin(HumidRelay_GPIO_Port, HumidRelay_Pin,
                      g_humidRelayOn ? GPIO_PIN_RESET : GPIO_PIN_SET);
  }
#endif
  ...
  while (1) {
    ...
    if (g_ntcRawAdc >=
        4090) /* near ADC_MAX -> open circuit / disconnected sensor */
    {
      g_ntcSensorFault = 1;
    } else {
      g_ntcSensorFault = 0;
       ...
       */
      g_ntcResistanceOhms = (uint32_t)(10000.0f * (float)g_ntcRawAdc /
                                       (float)(4095 - g_ntcRawAdc));
...
static uint8_t lcd_backlight_state =
    LCD_BACKLIGHT; /* current backlight bit, ORed into every write */
static void lcd_write4(uint8_t nibble, uint8_t rs) {
...
static void lcd_backlight_on(void) {
  ...
  lcd_i2c_write(lcd_backlight_state); /* backlight is just a static output pin —
                                         no Enable pulse needed */
}
