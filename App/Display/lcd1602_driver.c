...
static void lcd_i2c_write(uint8_t data) {
  (void)HAL_I2C_Master_Transmit(&hi2c1, LCD_I2C_ADDR, &data, 1u, 10u);
}
...
static void lcd_command(uint8_t command) { lcd_send(command, 0u); }

static void lcd_data(uint8_t character) { lcd_send(character, LCD_RS_BIT); }
