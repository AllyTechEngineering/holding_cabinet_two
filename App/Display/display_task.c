...
static void display_screen(UiScreen_t screen, uint16_t temp_value,
  ...
  if (screen == UI_IDLE_SPLASH) {
    ...
    LCD1602_WriteLines(" To Start Proof ", " Enter Y Mode N ");
      ...
      LCD1602_WriteLines(" Proof Complete ", " Press Mode to  ");
    }
    else if (screen == UI_COMPLETE_DISPLAY_B) {
      LCD1602_WriteLines(" Proof Complete ", " Start Again    ");
    }
    else {
  ...
  void DisplayTask_Run(void *argument) {
    ...
    for (;;) {
      ...
      if (osMessageQueueGet(qInputToDisplayHandle, &event, NULL,
        ...
        if (screen == UI_IDLE_SPLASH || screen == UI_IDLE_PROMPT) {
          ...
          if (event == EVT_MODE_PRESSED) {
            ...
            if (timed_proof != 0u) {
              started = RunTimer_StartTimed(
                            &run_timer, TimeEditor_Minutes(&time_editor), now)
                            ? 1u
                            : 0u;
            } else {
      ...
      if (screen == UI_RUN_ACTIVE && (uint32_t)(now - last_run_refresh_tick) >=
                                         APP_RUN_DISPLAY_REFRESH_MS) {
        last_run_refresh_tick = now;
