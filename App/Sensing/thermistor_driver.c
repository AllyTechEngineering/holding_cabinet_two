...
uint16_t Thermistor_ReadTenthsC(void)
{
  ...
  float r_ohms = NTC_R1_OHMS * (float)rawAdc / (float)(ADC_MAX_COUNT - rawAdc);
  float tempC  = ntc_resistance_to_celsius(r_ohms);

  /* Round to nearest tenth, not truncate. */
  int32_t tenths = (int32_t)lroundf(tempC * 10.0f);

  if (tenths < 0)
  {
    tenths = 0; /* defensive: shouldn't occur within this product's operating range */
  }
